...
<div class="d-flex align-items-center gap-2 mb-4">
    ...
    <h5 class="fw-700 mb-0">កែប្រែ: {{ $class->class_name }}</h5>
</div>
                ...
                <form method="POST" action="{{ route('classes.update',$class) }}">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="class_name" value="{{ old('class_name',$class->class_name) }}" class="form-control rounded-3 @error('class_name') is-invalid @enderror">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-500">លេខបន្ទប់</label>
                            <input type="text" name="room_number" value="{{ old('room_number',$class->room_number) }}" class="form-control rounded-3">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-500">គ្រូ (ប្រធានថ្នាក់)</label>
                            <select name="teacher_id" class="form-select rounded-3">
                                <option value="">— ជ្រើសរើស —</option>
                                @foreach($teachers as $teacher)<option value="{{ $teacher->id }}" {{ old('teacher_id',$class->teacher_id)==$teacher->id?'selected':'' }}>{{ $teacher->name }} — {{ $teacher->subject }}</option>@endforeach
                            </select>
                        </div>
