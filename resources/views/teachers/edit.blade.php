    ...
    <div class="row g-4">
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="teacher_id" value="{{ old('teacher_id',$teacher->teacher_id) }}" class="form-control rounded-3 @error('teacher_id') is-invalid @enderror" placeholder="TCH-001">
                            @error('teacher_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-500">ឈ្មោះ <span class="text-danger">*</span></label>
                            <input type="text" name="name" value="{{ old('name',$teacher->name) }}" class="form-control rounded-3 @error('name') is-invalid @enderror" placeholder="ឈ្មោះគ្រូ">
                        ...
                        <div class="col-md-6">
                            ...
                            @php
                                $selectedSubjects = array_map('trim', explode(',', old('subject', $teacher->subject ?? '')));
                            @endphp
                            <select name="subjects[]" class="form-select rounded-3 @error('subjects') is-invalid @enderror" multiple style="height: 100px;" required>
                                @foreach($courses as $course)
                                    <option value="{{ $course->course_name }}" {{ is_array(old('subjects', $selectedSubjects)) && in_array($course->course_name, old('subjects', $selectedSubjects)) ? 'selected' : '' }}>{{ $course->course_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" name="email" value="{{ old('email',$teacher->email) }}" class="form-control rounded-3 @error('email') is-invalid @enderror" placeholder="teacher@school.com">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="text-muted mt-1" style="font-size: .8rem;">ទុកទទេរ ប្រសិនបើមិនចង់ផ្លាស់ប្តូរ (រក្សាទុកពាក្យសម្ងាត់ចាស់)</div>
                            @error('password')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-500">ទូរស័ព្ទ</label>
                            <input type="text" name="phone" value="{{ old('phone',$teacher->phone) }}" class="form-control rounded-3" placeholder="012 345 678">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-500">អាសយដ្ឋាន</label>
                            <textarea name="address" rows="3" class="form-control rounded-3" placeholder="អាសយដ្ឋានរបស់គ្រូ...">{{ old('address',$teacher->address) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body p-4 text-center">
                    <div id="photoPreview" class="mb-3" style="width:120px;height:120px;border-radius:20px;background:#f1f5f9;margin:0 auto;overflow:hidden;border:2px dashed #cbd5e1;display:flex;align-items:center;justify-content:center;">
                        @if(!empty($teacher->photo))
                            <img src="{{ asset($teacher->photo) }}" style="width:100%;height:100%;object-fit:cover;">
                        @else
                            <i data-lucide="user" style="width:48px;height:48px;stroke:#94a3b8;"></i>
                        @endif
                    </div>
                    <label class="btn btn-outline-primary rounded-3 btn-sm" for="photoInput"><i data-lucide="upload" class="lucide-sm me-1"></i>ប្តូររូបថត</label>
            ...
            <div class="d-grid gap-2 mt-3">
                ...
                <a href="{{ route('teachers.show',$teacher) }}" class="btn btn-outline-secondary rounded-3"><i data-lucide="x-circle" class="lucide-sm me-2"></i>បោះបង់</a>
            </div>
        </div>
    </div>
</form>
